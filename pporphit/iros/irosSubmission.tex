...
% Stephen Gillet$^{*}$, Gilberto G.~Briscoe-Martinez, Alessandro Roncone% <-this % stops a space
% \thanks{$^{*}$Corresponding author.}% <-this % stops a space
% \thanks{GBM is supported by NASA Space Technology Graduate Research Opportunity Grant 80NSSC22K1211.}
% \thanks{All Authors with Department of Computer Science, University of Colorado Boulder, CO, USA.}%
...
But, with human workers, specialization is not just a matter of knowledge, but also of physical adaptation \cite{arany2008pgc1}\cite{mancuso2022effect}\cite{wu2020occupational}.
Why not then also adapt the physical embodiment of the robot to the task it is performing?
This paper seeks to specialize the morphological design of robots to specific tasks by using a PPO agent to optimize the morphology based on various performance metrics and task-specific physical constraints.
