...
The Panda approximation similarly taken from the product manual \cite{franka2024research3} has 7 links, lengths of 0.333, 0.316, 0.0825, 0.0825, 0.384, 0.107, 0.05 (meters), and joint types of 2, 0, 2, 0, 2, 0, 2 (revolute about z, x, z, x, z, x, z).
...
All tasks use a workspace centered at the robot base (0, 0, 0) except for the "Wall Mount" task where the base is moved to the middle of the near wall (0, -0.4, 1.0) and rotated $\pi/2$ about the x-axis to face outward from the wall.
The tasks are as follows:

\begin{enumerate}
    \item \textbf{Container Task}: The robot is positioned on the floor of a shipping-container-like environment in the middle and slightly to one side with walls and a ceiling (2m x 1m x 1m) all around and one open end. The start poses are near and facing the back of the container and the goal poses toward and near the open end (see Figure \ref{fig:containerTask}):
    ...
    \begin{table}[H]
        ...
        \hline
        Start 1 & [-1.8, 0.3, 0.3] & [0.7071, 0, -0.7071, 0] \\
        Goal 1 & [1.9, 0.9, 0.4] & [0.7071, 0, 0.7071, 0] \\
        Start 2 & [-1.8, 0.8, 0.4] & [0.7071, 0, -0.7071, 0] \\
        Goal 2 & [1.8, 0.31, 0.2] & [0.7071, 0, 0.7071, 0] \\
        \hline
        \end{tabular}
    \end{table}
    ...
    \begin{table}[H]
        ...
        \hline
        Back Wall & [-2.0, 0.6, 1.0] & [0.01, 1.0, 1.0] \\
        Left Wall & [0, -0.4, 1.0] & [2.0, 0.01, 1.0] \\
        Right Wall & [0, 1.6, 1.0] & [2.0, 0.01, 1.0] \\
        Ceiling & [0, 0.6, 2.0] & [2.0, 1.0, 0.01] \\
        \hline
        \end{tabular}
    \end{table}
    ...
    \begin{figure}[H]
    \centering
    \includegraphics[width=0.9\columnwidth]{wallMountTask.png}
    \caption{Wall Mount task.}
    ...
    \begin{table}[H]
        ...
        \textbf{Name} & \textbf{Position} & \textbf{Quaternion} \\
        \hline
        Start 1 & [-1.4, -0.4, 0.1] & [-0.7071, -0.7071, 0, 0] \\
        Goal 1 & [1.75, 1.36, 1.11] & [1, 0, 0, 0] \\
        Start 2 & [-0.9, 1.35, 1.1] & [1, 0, 0, 0] \\
        Goal 2 & [1.5, -0.4, 0.2] & [-0.7071, -0.7071, 0, 0] \\
        \hline
        \end{tabular}
    \end{table}
    ...
    \begin{table}[H]
        ...
        \textbf{Name} & \textbf{Position} & \textbf{Size} \\
        \hline
        Mount Wall & [0, -0.4, 1.0] & [1.0, 0.01, 1.0] \\
        Shelf Wall & [0, 1.6, 1.0] & [2.0, 0.01, 1.0] \\
        Shelf & [0.0, 1.35, 1.0] & [2.0, 0.25, 0.01] \\
        \hline
        \end{tabular}
    \end{table}
    ...
    \item \textbf{Shelf Task}: A single shelf obstacle is placed off to the side of the robot at 0.5m height.
    There are two sets of start and goal poses alternately above and below the shelf all facing in the positive x direction.
    ...
    \begin{table}[H]
        ...
        \textbf{Name} & \textbf{Position} & \textbf{Quaternion} \\
        \hline
        Start 1 & [1.1, -0.31, 0.6] & [0.7071, 0, 0.7071, 0] \\
        Goal 1 & [0.9, -0.71, 0.4] & [0.7071, 0, 0.7071, 0] \\
        Start 2 & [0.95, -0.29, 0.4] & [0.7071, 0, 0.7071, 0] \\
        Goal 2 & [1.05, -0.69, 0.6] & [0.7071, 0, 0.7071, 0] \\
        \hline
        \end{tabular}
    \end{table}
    ...
    \begin{table}[H]
        ...
        \textbf{Name} & \textbf{Position} & \textbf{Size} \\
        \hline
        Shelf & [1.0, -0.5, 0.5] & [0.25, 0.5, 0.01] \\
        \hline
        \end{tabular}
    \end{table}

    \item \textbf{Near-Base Outreach Task}: No obstacles; motion from near the base outward in one direction (e.g., positive x):

    \begin{table}[H]
        \centering
        \caption{Near-Base Outreach Task Poses}
        \label{tab:near-base-poses}
        \begin{tabular}{lll}
        \hline
        \textbf{Name} & \textbf{Position} & \textbf{Quaternion} \\
        \hline
        Start & [0.1, 0, 0.1] & [1, 0, 0, 0] \\
        Goal & [1.0, 0, 0.5] & [1, 0, 0, 0] \\
        \hline
        \end{tabular}
    \end{table}

    \item \textbf{Side-to-Side Task}: No obstacles; poses on either side of the base (e.g., left and right in y-direction):

    \begin{table}[H]
        \centering
        \caption{Side-to-Side Task Poses}
        \label{tab:side-to-side-poses}
        \begin{tabular}{lll}
        \hline
        \textbf{Name} & \textbf{Position} & \textbf{Quaternion} \\
        \hline
        Start 1 & [0, -0.5, 0.3] & [1, 0, 0, 0] \\
        Goal 1 & [0, 0.5, 0.3] & [1, 0, 0, 0] \\
        Start 2 & [0, 0.5, 0.5] & [1, 0, 0, 0] \\
        Goal 2 & [0, -0.5, 0.5] & [1, 0, 0, 0] \\
        \hline
        \end{tabular}
    \end{table}
\end{enumerate}

For broader validation, we report results on all tasks, with the container task as the primary benchmark.

\subsection{Baselines}

We compare against two fixed industrial morphologies:
\begin{itemize}
    \item \textbf{FANUC LR Mate 200iD} (most common 6-DoF model, 70,000+ installed \cite{fanuc2024lrmate}): \(n=6\), lengths \([0.165, 0.330, 0.08, 0.285, 0.05, 0.05]\) m (scaled from official datasheet), joint types \([2, 0, 0, 2, 0, 2]\) (all revolute).
    \item \textbf{Franka Panda} (7-DoF research standard): \(n=7\), lengths \(2\times[0.333, 0.316, 0.0825, 0.0825, 0.384, 0.088, 0.01]\) m, joint types \([2, 1, 2, 0, 2, 0, 2]\).
\end{itemize}

All baselines use identical IK, planning, and reward computation for fair comparison.

\subsection{Metrics}

Performance is measured by the average reward \(r(\theta)\) over 100 test episodes (with fresh noise). We also report component-wise breakdowns:
\begin{itemize}
    ...
    \item Link count \(n\).
\end{itemize}
Results are averaged over 3 random seeds with 95\% confidence intervals.

\subsection{Implementation and Compute}

Training used Stable Baselines3 PPO (1M timesteps, single-step episodes) on an NVIDIA RTX 4060 GPU (or CPU fallback: 11th Gen Intel i9-11900KF). The environment was vectorized with 24 parallel instances (`SubprocVecEnv`). Full hyperparameters are in Table~\ref{tab:hyperparams}.
...
\end{tabular}
\end{table}
All code, models, and logs are available in the supplementary material.
