...
To evaluate PMorph, we conducted experiments in the MuJoCo simulation environment (version 3.3.7) using the container task defined in Section~\ref{sec:task}. All training and evaluation used the same evaluation pipeline (XML generation, robust DLS IK, RRT-Connect) and reward function from Section~\ref{sec:eval}.

\subsection{Tasks}

The primary task is the \emph{container task} (Fig.~\ref{fig:containerTask}), which models a realistic manufacturing pick-and-place scenario inside a confined workspace (2m × 1m × 1m) with fixed obstacles (walls, ceiling, floor). The robot must transport objects between two noisy start--goal pose pairs:
\begin{itemize}
    \item Start 1: \([-1.8, 0.3, 0.3, 0.7071, 0, -0.7071, 0]\)
    \item Goal 1: \([1.9, 0.9, 0.4, 0.7071, 0, 0.7071, 0]\)
    \item Start 2: \([-1.8, 0.8, 0.4, 0.7071, 0, -0.7071, 0]\)
    \item Goal 2: \([1.8, 0.31, 0.2, 0.7071, 0, 0.7071, 0]\)
\end{itemize}
Poses are corrupted with Gaussian noise \(\sigma=0.05\) to promote robust morphologies. 

For broader validation, we also tested on three variants (from the original task set):
\begin{enumerate}
    \item \textbf{Basic}: Linear motion at average height/distance.
    \item \textbf{Shelf}: Vertical motion with intermediate obstacle.
    \item \textbf{Multi-task}: Four opposing motions (near/far, low/high).
\end{enumerate}
