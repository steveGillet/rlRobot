...
Figure \ref{fig:sacContainerArm} shows the SAC generated morphology for the container task and it is significantly more simple than the generic arms but also significantly less accurate.

\chapter{Conclusion}

The results were promising at first and remain so in simple tasks, but as the tasks become more constrained the simple controller starts failing to find paths, especially for higher link number arms which biases the agent towards selecting simpler arms which can't score highly but can consistently get paths.
