...
def main():
    ...
    print(f"\n🤖 Robot 1 (Config {current_config})")
    print(f"   Links: [{lengths[0]:.2f}, {lengths[1]:.2f}, {lengths[2]:.2f}, {lengths[3]:.2f}] = {np.sum(lengths):.2f}m")
    
    print("\n🎬 Starting visualization - viewer will stay open!")
    ...
    try:
        while viewer.is_running():
            ...
            if current_time - last_change >= interval:
                ...
                print(f"\n{'='*70}")
                print(f"🤖 Robot {robot_count} (Config {current_config}) - VIEWER STAYS OPEN!")
                print(f"   OLD: [{old_lengths[0]:.2f}, {old_lengths[1]:.2f}, {old_lengths[2]:.2f}, {old_lengths[3]:.2f}] = {np.sum(old_lengths):.2f}m")
                print(f"   NEW: [{new_lengths[0]:.2f}, {new_lengths[1]:.2f}, {new_lengths[2]:.2f}, {new_lengths[3]:.2f}] = {np.sum(new_lengths):.2f}m")
