...
\begin{enumerate}
    ...
    \vspace{1em}
    \includegraphics[width=0.75\textwidth]{heatmap_lambda_0.7.png}
    
    \vspace{1em}
    \includegraphics[width=0.75\textwidth]{heatmap_lambda_0.95.png}
    
    \item \textit{Describe how the "trail" of eligibility changes as you increase $\lambda$. Why does the credit propagate further back in time for higher values?}
    
    The higher the lambda value the longer the trail. Lambda dictates the size of the eligibility trace scaling after each step, so the larger it is the less the traces are scaled down. The effect of that is that when lambda is small the values of traces get scaled down to miniscule values after a couple of steps so you only see the last couple of trace values.
